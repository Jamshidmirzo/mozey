...
import { REGION_POS, type MuseumItem } from '@/lib/constants';
...
interface MapSectionClientProps {
  /** Already-fetched live items from API; mix of museums + historical places. */
  items: MuseumItem[];
}

export function MapSectionClient({ items }: MapSectionClientProps) {
  ...
  // Empty-state — DB has nothing to render on the map yet.
      ...
      >
        <div className="text-center mb-9">
          ...
          <h2
            ...
            style={{ fontSize: 'clamp(32px, 4.4vw, 48px)', letterSpacing: -0.8 }}
          >
            {t('title')}
          </h2>
          <p className="font-ui text-[17px] leading-[1.6] text-ink2 max-w-[560px] mx-auto mt-[14px]">
            Скоро откроется — данные ещё готовятся в админке.
          </p>
        </div>
      </section>
    );
